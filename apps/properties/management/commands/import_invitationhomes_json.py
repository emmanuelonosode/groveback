...
DEFAULT_JSON_PATH = Path(
    r"C:\Users\emman\Documents\Codex\2026-05-18\base-on-our-conversation-on-invitationhome"
    r"\data\invitationhomes_properties_latest.json"
)
...
def _discounted_rent(value):
    ...
    if original <= 0:
        return Decimal("0")
    return ((original * Decimal("0.80")) / Decimal("100")).quantize(
        Decimal("1"),
...
def _photo_url(photo):
    ...
    if not isinstance(photo, dict):
        return ""
    for key in ("url", "src", "image_url", "imageUrl", "originalUrl", "cdnUrl", "largeUrl"):
        value = photo.get(key)
...
class Command(BaseCommand):
    help = "Import the scraped Invitation Homes JSON into the dev database."

    def add_arguments(self, parser):
        parser.add_argument(
            "--json",
            default=str(DEFAULT_JSON_PATH),
            help="Path to invitationhomes_properties_latest.json.",
        )
        parser.add_argument(
            "--clear",
            action="store_true",
            help="Delete existing JSON-imported Invitation Homes rows before importing.",
        )
        parser.add_argument(
            "--markets",
            default=None,
            help="Comma-separated market slugs to import, e.g. atlanta-georgia,tampa-florida.",
        )
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            total_images = sum(len((item.get("summary") or {}).get("photos") or []) for item in selected)
            total_amenities = sum(len((item.get("summary") or {}).get("amenities") or []) for item in selected)
            self.stdout.write(self.style.WARNING("DRY RUN - nothing written."))
            self.stdout.write(f"Images found: {total_images}")
            self.stdout.write(f"Amenities found: {total_amenities}")
            return
        ...
        with transaction.atomic():
            ...
            for item in selected:
                ...
                prop = Property.objects.create(
                    ...
                    garage=1 if any("garage" in _amenity_name(a).lower() for a in (summary.get("amenities") or [])) else 0,
                    ...
                    is_featured=bool(summary.get("flags", {}).get("is_featured_listing")),
                    is_published=True,
                )
                ...
                image_urls = [_photo_url(photo) for photo in photos]
                total_images += _insert_images_raw(prop.id, image_urls)

                amenity_objects = []
                seen_amenities = set()
                for amenity in summary.get("amenities") or raw.get("amenities") or []:
                    ...
                    amenity_objects.append(PropertyAmenity(property=prop, category=category, name=name))
                if amenity_objects:
